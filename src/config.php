...
// === XUI/Sanaei compatibility helpers (API v1.7.9+ and 3x-ui latest) ===
if (!function_exists('xui_login')) {
    /**
     * Login and return cookie string for Authorization.
     * Works for both legacy and latest 3x-ui.
     */
    function xui_login($panel_url, $username, $password) {
        ...
        // Fallback to the first cookie
        if (!empty($cookies)) {
            $k = array_key_first($cookies);
            return $k.'='.$cookies[$k];
        }
        return null;
    }
    ...
    /**
     * Detects whether the new /panel/api routes are available.
     * Returns true if HTTP 200 and JSON body, else false.
     */
    function xui_supports_new_api($panel_url, $cookieHeader) {
        $url = rtrim($panel_url, '/').'/panel/api/inbounds';
        $ch = curl_init();
    ...
    /**
     * Build the correct Update Inbound endpoint for both API styles.
     */
    function xui_update_inbound_url($panel_url, $id, $newApi) {
        return rtrim($panel_url, '/').($newApi ? "/panel/api/inbounds/update/$id" : "/xui/inbound/update/$id");
    }

    /**
     * Delete client endpoint may differ: New API uses POST /panel/api/inbounds/delClient with json body {id, clientId/email/uuid}.
     * Legacy uses POST /panel/inbound/{id}/delClient/{uuid}
     */
    function xui_delete_client($panel_url, $cookieHeader, $inbound_id, $uuid, $dataArr = []) {
        $newApi = xui_supports_new_api($panel_url, $cookieHeader);
        $ch = curl_init();
        if ($newApi) {
            $url = rtrim($panel_url, '/')."/panel/api/inbounds/delClient";
            $payload = [
                "id" => (int)$inbound_id,
                "client" => $uuid,
            ];
            ...
            $url = rtrim($panel_url, '/')."/panel/inbound/$inbound_id/delClient/".rawurlencode($uuid);
            curl_setopt_array($ch, [
                CURLOPT_URL => $url,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_CONNECTTIMEOUT => 10,
    ...
    /**
     * Add client endpoint wrapper: new API uses /panel/api/inbounds/addClient
     * Legacy flow edits the entire inbound with updated settings.
     */
    function xui_add_client($panel_url, $cookieHeader, $inbound_id, $clientSettingsJson, $legacyUpdatePayload) {
        $newApi = xui_supports_new_api($panel_url, $cookieHeader);
        $ch = curl_init();
        if ($newApi) {
            $url = rtrim($panel_url, '/')."/panel/api/inbounds/addClient";
            $payload = [
                "id" => (int)$inbound_id,
                "settings" => $clientSettingsJson, // stringified JSON per 3x-ui
            ];
            curl_setopt_array($ch, [
                CURLOPT_URL => $url,
                CURLOPT_RETURNTRANSFER => true,
            ...
            $url = xui_update_inbound_url($panel_url, $inbound_id, false);
            curl_setopt_array($ch, [
                CURLOPT_URL => $url,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_CONNECTTIMEOUT => 10,
                ...
                CURLOPT_POSTFIELDS => $legacyUpdatePayload,
            ]);
        }
        $res = curl_exec($ch);
        $err = curl_error($ch);
...
function deleteClient($server_id, $inbound_id, $uuid, $delete = 0){
    ...
    if($delete == 1){
        ...
        if($serverType == "sanaei" || $serverType == "alireza"){
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => xui_update_inbound_url($panel_url, $inbound_id, xui_supports_new_api($panel_url, '' )),
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_CONNECTTIMEOUT => 15,  
...
function editInboundRemark($server_id, $uuid, $newRemark){
    ...
    else $url = xui_update_inbound_url($panel_url, $inbound_id, xui_supports_new_api($panel_url, '' ));
...
function addInboundAccount($server_id, $client_id, $inbound_id, $expiryTime, $remark, $volume, $limitip = 1, $newarr = '', $planId = null){
    ...
    if($serverType == "sanaei" || $serverType == "alireza"){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => xui_update_inbound_url($panel_url, $iid, xui_supports_new_api($panel_url, '' )),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_CONNECTTIMEOUT => 15,
...
function updateConfig($server_id, $inboundId, $protocol, $netType = 'tcp', $security = 'none', $rahgozar = false){
  ...
  "grpcSettings": {
    ...
    else $url = xui_update_inbound_url($panel_url, $iid, xui_supports_new_api($panel_url, '' ));
    curl_setopt_array($curl, array(
        CURLOPT_URL => $url,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
